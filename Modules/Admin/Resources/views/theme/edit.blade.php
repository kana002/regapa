@extends('admin::layouts.master', ['edit_theme' => 'active bg-secondary rounded'])

@section('content')

<div class="container px-5 pt-5">
<form action="{{route('admin.theme.update',  ['id' => request()->route('id')] )}}" method="post">
        @csrf
        <div class="row">
            ...
            <div class="form-group col-4">
                ...
                <input name="col_chas" type="number" class="form-control"
                    ...
                    value="{{$themes->col_chas}}" required
                >
            </div>

   
      <div class="form-group col-4">
    <label for="id" class="text-primary">Тип</label>
    <select
                    class="form-select"
                    name="type"
                    required
                ...
                @foreach($typs as $type)
						<option
                            @if($themes->type == $type->id) selected @endif
                            value="{{$type->id}}"
                            >{{$type->name_ru}}</option>
					@endforeach
          </select>
            ...
            <div class="form-group col-4">
				...
				<input type="date" class="form-control" id="exampleCheck1" name="date_start" value="{{$themes->date_start}}">
			</div>
            <div class="form-group col-4">
				<label for="date_end">Дата окончания</label>
				<input type="date" class="form-control" id="exampleCheck1" name="date_end" value="{{$themes->date_end}}">
			</div>
