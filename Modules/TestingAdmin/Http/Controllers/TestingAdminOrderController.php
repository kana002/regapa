...
class TestingAdminOrderController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $all_orders = TestingOrder::where('reg_id', $user->region_id)->with('user')->get();

        $all_orderss = TestingOrder::where('reg_id', $user->region_id)->with('ready_order')->get();
        //dd($all_orderss);
        //$testing_order_doc = TestingOrderDoc::where('testing_order_id', $user->region_id)->get();


         return view('testingadmin::order.index', compact('user','all_orders'));
    ...
    public function create()
    {
        ...
        return view('testingadmin::order.create', compact('user','all_orders'));
    ...
    public function store(Request $request)
    {
       ...
       //dd($data);
       return redirect()->route('testingadmin::order.index');
    }
    ...
    public function show($id, Request $request)
    {
        ...
        return view('testingadmin::order.show',  compact('user','all_orders'));
    }
    ...
    public function edit($id, Request $request)
    {
        ...
        $all_orders = TestingOrder::where('id', $id)
                                ->with('user')
                                ->with('details')
                                ->first();
        //$testing_order = TestingOrder::find($id);
        ...
        if($all_orders != null)
        {
            ...
            return $pdf->stream($all_orders->test_gen.'_order.pdf');
    ...
    {
        $user = Auth::user();
        $all_orders = TestingOrder::where('reg_id', $user->region_id)->get();
       //dd($id);

        $rr =  TestingOrder::where('id', $id)->update([
            'status_id' => 2,
            'reason' => $request->reason
        ]);

          return redirect()->route('testingadmin.all_orders',  compact('all_orders'));
    ...
    public function delete(Request $request, $id)
    {
       ...
       //dd($id);
        ...
        $rr =  TestingOrder::where('id', $id)->delete();
