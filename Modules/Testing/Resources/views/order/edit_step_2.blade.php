	...
	<form action="{{route('testing.new_order.store_step_2', ['test_gen' => request()->get('test_gen')])}}" method="post">
		...
		<div class="row pt-4">
			<div class="form-group col-4">
				<label>{{ __('modules.test_category') }}</label>
				<select
                    class="form-select "
                    name="test_category"
                    required
			...
			<div class="form-group col-4">
				...
				<select
					...
					@foreach($regions as $region)
						<option
							@if($testing_order->reg_id == $region->id) selected @endif
							value="{{$region->id}}">{{$region->$locale}}</option>
					@endforeach
			...
			<div class="form-group col-4">
				...
				{{--<input value="{{$testing_order->desired_test_date ?? ''}}" name="test_desired_date" type="date" class="form-control" id="test_desired_date">--}}
				<input
					id="test_chosen_date"
					name="test_desired_date"
					value="{{$testing_order->desired_test_date ?? ''}}"
			...
			<div class="form-group col-4">
                ...
                >
                    @foreach($langs as $lang)
						<option
							@if($testing_order->lang_id == $lang->id) selected @endif
							value="{{$lang->id}}">{{$lang->$locale}}</option>
					@endforeach
                </select>
			</div>
		</div>
		...
		<div class="row pt-4 gx-0">
			<div class="col border border-info rounded bg-light shadow p-3" id="docs_list_div">
				...
				<ul id="docs_list">
					...
					@if(isset($testing_order->category_id))
						@if($testing_order->category_id == '1')
							<li>{{ __('modules.cert_pm_35_hours') }}</li>
						@elseif($testing_order->category_id == '2')
							<li>{{ __('modules.cert_pm_level_1') }}</li>
						@elseif($testing_order->category_id == '3')
							<li>{{ __('modules.cert_pm_level_1_2') }}</li>
						@else
							<li>{{ __('modules.cert_pm_level_1_2_3') }}</li>
						@endif
					@else
						<li>{{ __('modules.cert_pm_35_hours') }}</li>
					@endif
