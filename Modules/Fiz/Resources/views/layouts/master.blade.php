			...
			<div class="container-fluid">
				...
				<div class="collapse navbar-collapse justify-content-end me-2" id="navbarNav">
					<ul class="navbar-nav">
						...
						<li class="nav-item">
							<a class="nav-link {{ $home ?? '' }}" target="_blank" aria-current="page" href="{{route('fiz.home')}}">Главная</a>
						</li>
						<li class="nav-item">
							<a class="nav-link {{ $new_order ?? '' }}" target="_blank" href="{{route('fiz.new_order.step_1')}}">Новая завка</a>
						</li>
						<li class="nav-item">
							<a class="nav-link {{ $all_orders ?? '' }}" target="_blank" href="{{route('fiz.all_orders')}}">Все заявки</a>
						</li>
						<li class="nav-item dropdown">
							...
							<ul class="dropdown-menu" aria-labelledby="navbarScrollingDropdown">
								<li>
									<a class="dropdown-item {{ $settings ?? '' }}" target="_blank" href="{{route('fiz.settings')}}">Настройки</a>
								</li>
					...
					<div class="d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 text-white">
						...
						<ul class="nav nav-pills flex-column mb-sm-auto mb-0 align-items-center align-items-sm-start" id="menu">
							<li class="nav-item w-100 border-bottom">
								<a href="{{Route::is('fiz.edit_order.*')
										? route('fiz.edit_order.step_1', ['gen'=>request()->session()->get('cur_gen')])
										: route('fiz.new_order.step_1')}}"
							...
							<li class="nav-item w-100 border-bottom">
							<a href="{{Route::is('fiz.edit_order.*')
										? route('fiz.edit_order.step_2', ['gen'=>request()->session()->get('cur_gen')])
										: route('fiz.new_order.step_2')}}"
									class="nav-link align-middle px-0">
							...
							<li class="nav-item w-100 border-bottom">
								<a href="{{Route::is('fiz.edit_order.*')
										? route('fiz.edit_order.step_3', ['gen'=>request()->session()->get('cur_gen')])
										: route('fiz.new_order.step_3')}}"
									class="nav-link align-middle px-0">
							...
							<li class="nav-item w-100">
								<a href="{{request()->session()->get('cur_gen') != null
										? route('fiz.order.step_4', ['gen'=>request()->session()->get('cur_gen')])
										: ''}}"
									class="nav-link align-middle px-0">
