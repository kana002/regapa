...
class GovOrderController extends Controller
{
    public function index()
    {
        ...
        //$themes = Theme::all();
        return view('gov::order.index', compact('user', 'orders'));
    }
    ...
    public function create_step_1()
    {
        ...
        return view('gov::order.create_step_1', compact('user', 'job_categories', 'job_orgs', 'vaccines'));
    }
    ...
    public function create_step_2(Request $request)
    {
        ...
        return view('gov::order.create_step_2', compact('user', 'langs', 'themes', 'theme_types'));
    }
    ...
    public function create_step_3(Request $request)
    {
        ...
        return view('gov::order.create_step_3', compact('user'));
    }
    ...
    public function create_step_4(Request $request)
    {
        ...
        $user = Auth::user();
        $order = Order::where('gen', $request->gen)->first();
        return view('gov::order.create_step_4', compact('user', 'order'));
    }
        ...
        if ($request->hasFile('photo')) {
                ...
                if($exists)
                {
                    ...
                    if($order == null)
                    {
                        ...
                        return Redirect::route('gov.new_order.step_2', ['gen' => $new_order->gen])->with('message', 'Saved new');
                    }
                }
    ...
    public function store_step_2(Request $request, $gen)
    {
        ...
        if($order->status_id == '0')
        {
            ...
            return Redirect::route('gov.new_order.step_3', ['gen' => $gen])->with('message', 'Saved new');
        }
    ...
    public function store_step_3(Request $request, $gen)
    {
        ...
        if($order->status_id == '0')
        {
            ...
            if($referer_route == 'testing.edit_order.step_2') return Redirect::route('testing.edit_order.step_3', ['test_gen' => $test_gen])->with('message', 'Saved existing');*/
            //else return Redirect::route('testing.new_order.step_3', ['test_gen' => $test_gen])->with('message', 'Saved existing');
            return Redirect::route('gov.order.step_4', ['gen' => $gen])->with('message', 'Saved new');
        }
        else return 'Невозможно отредактировать данную заявку.';
    }
    ...
    public function edit_step_1(Request $request)
    {
        ...
        return view('gov::order.edit_step_1', compact('user', 'job_categories', 'job_orgs', 'vaccines', 'order'));
    }
    ...
    public function edit_step_2(Request $request)
    {
        ...
        return view('gov::order.edit_step_2', compact('user', 'langs', 'themes', 'theme_types', 'order'));
    }
    ...
    public function edit_step_3(Request $request)
    {
        ...
        return view('gov::order.edit_step_3', compact('user', 'order'));
    }
        ...
        if ($request->hasFile('photo'))
        {
            ...
            if($exists)
            {
                ...
                if($order)
                {
                    ...
                    return Redirect::route('gov.edit_order.step_2', ['gen' => $gen])->with('message', 'Saved new');
                }
    ...
    public function update_step_2(Request $request, $gen)
    {
        ...
        if($order->status_id == '0')
        {
            ...
            return Redirect::route('gov.edit_order.step_3', ['gen' => $gen])->with('message', 'Saved existing');
        }
        else return 'Невозможно отредактировать данную заявку.';
    }
    ...
    public function update_step_3(Request $request, $gen)
    {
        ...
        if($order->status_id == '0')
        {
            ...
            return Redirect::route('gov.order.step_4', ['gen' => $gen])->with('message', 'Saved existing');
        }
        else return 'Невозможно отредактировать данную заявку.';
    }
    ...
    public function getReadyOrder(Request $request, $gen)
    {
        $user = User::where('id', Auth::user()->id)->with('details')->first();
        $order = Order::where('gen', $gen)->first();
        $pdf = PDF::loadView('gov::order.ready_order_template', compact('user', 'order'))
        ...
        $new_path_order = 'orders/ready_orders/gov/'.$user->id
